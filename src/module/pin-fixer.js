...
export class PinFixer {
	/** @type {Number} */
	static get minCanvScale() {
		return 0.1;
	}
	/** @type {Number} */
	static get maxCanvScale() {
		return CONFIG.Canvas.maxZoom;
	}
	...
	static get lockPins() {
		return this.enabled && this.pinLocker && !this.onNotesLayer;
	}
	...
	static scaleNotes(scale) {
		...
		canvas.notes.objects.children.forEach((note) => this.scaleNote(note, scaled));
	}
	/**
	...
	static init(...args) {
		loadTemplates([
			'modules/pin-fixer/templates/sceneSettings.html',
			//"modules/pinfixer/templates/.html"
		]);
	}
	...
	static attachEventListeners(html) {
		...
		slideWrapper.find('.range-slider').change((event) => this.inputToInput(event, '.range-editor'));
		slideWrapper.find('.range-editor').change((event) => this.inputToInput(event, '.range-slider'));
	}
	...
	static async renderSceneConfig(sceneConfig, html, data) {
		...
		ambItem.after(
			`<a class="item" data-tab="pin-fixer"><i class="fas fa-bookmark"></i> ${game.i18n.localize(
				'pin-fixer.title',
			)}</a>`,
		);
	...
	static async renderNoteConfig(noteConfig, html, data) {
		...
		html
			.find('.form-group')
			.last()
			.after(await this.getNoteHtml(this.getNoteTemplateData(data)));
		this.attachEventListeners(html);
	}
	...
	static async getSceneHtml(settings) {
		return await renderTemplate('modules/pin-fixer/templates/sceneSettings.html', settings);
	}
	...
	static async getNoteHtml(settings) {
		return await renderTemplate('modules/pin-fixer/templates/noteSettings.html', settings);
	}
